...
In a nutshell, it is resource intensive to achieve an optimal control over target device either by open-loop or closed-loop approach. With closed-loop approach, the balance of exploration and exploitation has to be chose and keep in mind that this is happen on the real device. With open-loop approach, the accuracy of characterized model is prominent. Thus, needs of experiment to characterize target device still crucial and potentially costly. Characterized model can be used beyond the optimal control problem, further information can be extracted, and interaction cost with target device can be reduced by interact with model first in the early stage of development.
...
Through this section, I will discuss how can we approach the problem, the ways forward to data-efficiency. Starting from characterization of quantum device using statistical machine learning way. Then moving forward to probabilistic machine learning way. Building upon the solid foundation of statistical version of model, then equip it with uncertainty quantification capability with probabilistic version. Finally, the model can be used with @boed, the principled framework for efficient utilization of data @fosterVariationalBayesianOptimal2019

Machine learning is a field of study to model a system of interested without explicitly program it to perform prediction by inferring from an observed data. In this thesis, we would like to categorize the field into statistical and probabilistic approach. To bring in our context, we interested in regression program since we are dealing the problem of quantum device characterization to construct a *predictive model*. For the @sml, we consider the model that learn by minimizing some objective function calculated from the observed data. While, the @pml, we infer the predictive model from the observed data by using Bayesian inference.

@sml is mature field that find its success in many fields. Compare to the @pml, it is a faster and more efficient way to construct a predictive model. However, it does not explicitly quantify the prediction uncertainty which is crucial in many appplications such as self-driving, medical application, and etc. On the other hand, probabilistic approach is a mathematical formulation of uncertainty @henningProbabilisticMachineLearning2023. It is also a common language that we can use to connect with other fields @p.murphyProbabilisticMachineLearning2022. Moreover, mpirical Risk Minimization (ERM) is equivalent to Maximum a posterior (MAP) which give a natural connection between @sml and @pml. Nonetheless, we can learn from @sml approach and use the insights gained from @sml to implement the @pml appraoch which we might view it as a natural way of using the @sml as a prior for the @pml. For example, we can use @sml as a baseline reference for the @pml implementation.
...
#tldr[
  The data-efficient is generally involes quantifying the current (prior) knowledge of the system, estimate some utility value, such as expected information gain, based on the prior knowledge, and collect data based on the estimated value. The steps are repeatedly performed until some conditions are met.
]

From the discussion in @sec:data-efficient-notion, there are a number of ways to reduce the consumption of the data in the model characterization (not necessary specific to quantum system). Here, we are going to discuss them in the context of quantum device characterization and a possible uses of the @eq:var-data to reduce the data consumption.

=== One-step Optimal

As discuss in @sec:data-efficient-notion, we can calculate @eig using estimators proposed in @fosterVariationalBayesianOptimal2019 and simply select the control parameters that maximize the @eig as the next design for the experiment. This procedure is referred as one-step optimal in the information-theoric perspective.

In @fosterVariationalBayesianOptimal2019, they demonstrated the example use case of @eig with the memory capacity experiment. The concept is, however, general. In our context, we can also apply the use of @eig as well. Thus, to estimate the @eig, we need a probabilistic model of the quantum system. Fortunately, we have constructed and discuss such a probabilistic model in @sec:pml-way. There are wide classes of probabilistic model that can model the quantum system. For instance, from the parametrized Hamiltonian, we can identify some or all of the system parameters (qubit frequency, detuning, and etc.) to be random variables. We then apply the @boed to identify those system parameters. Alternatively, we can also consider the probabilistic Graybox model where we promote the @dnn to @bnn. The model parameters of the @bnn which are the weigths of the model are the random variables. So, we apply the @boed to identify those weights. Either ways, we expect that with @boed approach, we can identify the target parameters with less experiment than the traditional appraoches.

=== Sequential-BOED

We have discussed from the results presented in previous works that one-step @boed is not necessary yield the highest total @eig in the sequential experiments setting. Successive works proposed the seqeuntial-BOED approaches which aim to maximize total @eig rather than a single-step @eig. However, intensive precomputation are required.

To handle the computational intensive requirement of the schemes developed for seqeuntial-BOED @fosterDeepAdaptiveDesign2021 @ivanovaImplicitDeepAdaptive2021, we can strategize the selection of experimental designs based on our priors on the problem. For demonstration propose, we will consider one of the possibe strategies in @sec:boed-experiment, namely, a *subspace strategy* that divides control parameters space into multiple subspaces, then select the designs that maximize the @eig within the subspace. The strategy is motivated by the fact that we want to perform inference for @bnn (the Blackbox part of the Graybox model). Hence, we want to retain some balance (to not fully bias the selection to @eig) in our dataset.

It is also possible to use `inspeqtor` to perform sequential-BOED as proposed in @fosterDeepAdaptiveDesign2021 @ivanovaImplicitDeepAdaptive2021, however, we do not explore them numerically in this thesis since they require too much computational resource.
// #margin-note()[The prototype code take too much resource to run]

=== Data-variance approach <sec:data-variance-approach>

From @eq:var-data, we can estimate the optimality of the predictive model. In the case of statistical predictive model, we can use its prediction directly. While, in the case of probabilistic model, we have to use the mean of the prediction as a prediction representation. Either ways, we can perform experiments to construct the dataset in mini batches. For each iteration, we use all of the dataset collected so far to characterization the model. Then, with the testing dataset, we can estimate the optimality of the model in which we can stop the data collection when some thresholds are reached.
